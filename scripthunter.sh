...
cat $TMPDIR/results/scripts-$domain.txt | httpx -H "cookie: $cookie" -status-code -silent -no-color | grep -E '\[200\]$' | cut -d " " -f1 | tee -a $TMPDIR/results/scripts-200-$domain.txt
